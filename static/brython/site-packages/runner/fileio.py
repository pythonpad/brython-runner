...
class PythonpadTextIOWrapper(io.IOBase):
    ...
    def fileno(self):
        raise OSError(
            'no file descriptor is available in simulated in-memory file system')
...
class PythonpadBytesIOWrapper(io.BufferedIOBase):
    ...
    def flush(self):
        ...
        cursor = self.stream.tell()
        self.stream.seek(0)  # Seek to the beginning of the stream.
        self.target_file['body'] = base64.b64encode(
            self.stream.read()).decode('utf-8')
        files_updated(self.filename)
        self.stream.seek(cursor)
...
def files_updated(path):
    if path in browser.self.files:
        browser.self.filesUpdated(
            path, browser.self.files[path]['type'], browser.self.files[path]['body'])
    else:
...
def normalize_path(path):
    ...
    if '/' in normalized_path:
        raise NotImplementedError(
            'directory structure is not supported in Pythonpad')
...
def open(file, mode='r', buffering=-1, encoding=None, errors=None, newline=None, closefd=True, opener=None):
    ...
    if 'b' in mode:
        ...
        if target_file['type'] != 'base64':
            raise NotImplementedError(
                'opening text file in bytes mode is not implemented in Pythonpad')
        ...
        if target_file['type'] != 'text':
            raise NotImplementedError(
                'opening byte file in text mode is not implemented in Pythonpad')
